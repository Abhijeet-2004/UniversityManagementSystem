...
public class UpdateTeacher extends JFrame implements ActionListener {
	...
	UpdateTeacher() {
		...
		lblEmpid .setBounds(50, 50, 150, 20);
		add(lblEmpid );
		...
		try {
			...
			while (rs.next()) {
				...
				tfCourse.setText(rs.getString("HighestQualification"));
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == update) {
			...
			Connection con=Conn.getConnection();
			String query="Update teacher set address=?,phone=?,email=?,HighestQualification=?,department=? where empid=?";
